...
public class GameFlowController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            LogDebug("未检测到存档恢复，加载起始Stage");
            LoadStage(startingStageId);
        }
        else if (isRestoringFromSave)
        {
            LogDebug("检测到存档恢复模式，跳过自动加载起始Stage");
        }
    }
    ...
    /// <param name="triggerAutoSave">是否触发自动存档（默认true）</param>
    public void LoadStage(string stageId, bool triggerAutoSave = true)
    {
        LogDebug($"加载Stage: {stageId}, 触发存档: {triggerAutoSave}");
    ...
    /// <param name="isFromSave">是否来自存档恢复（默认true）</param>
    public void RestoreState(GameFlowState state, bool isFromSave = true)
    {
        ...
        LogDebug($"恢复游戏流程状态 (从存档: {isFromSave})");
